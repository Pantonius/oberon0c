...
Scanner::Scanner(const path &path, Logger &logger)
    : path_(path), logger_(logger), lineNo_(1), charNo_(0), ch_{}, eof_(false) {
  init();
...
unique_ptr<const Token> Scanner::scanIdent() {
  ...
  if (const auto it = keywords_.find(ident); it != keywords_.end()) {
    if (it->second == TokenType::boolean_literal) {
      return make_unique<BooleanLiteralToken>(pos, current(),
                                              it->first == "TRUE");
    }
...
unique_ptr<const Token> Scanner::scanNumber() {
  ...
  while (!eof_ && ((ch_ >= '0' && ch_ <= '9') ||
                   (std::toupper(ch_) >= 'A' && std::toupper(ch_) <= 'F'))) {
    ss << ch_;
    read();
    if (ch_ == '.') {
  ...
  if (isFloat) {
    ...
    try {
      ...
      } else if (auto result = boost::convert<float>(
                     num, ccnv(std::dec)(std::scientific))) {
        // Check whether value was too small to represent as float and has been
        // rounded to zero
        if (result.value() == 0) {
          value = boost::convert<double>(num, ccnv(std::dec)(std::scientific))
                      .value();
          if (value != 0) {
      ...
      value =
          boost::convert<double>(num, ccnv(std::dec)(std::scientific)).value();
      return make_unique<DoubleLiteralToken>(pos, current(), value);
  ...
  if (isInt) {
    return make_unique<IntLiteralToken>(pos, current(),
                                        static_cast<int32_t>(value));
  }
  return make_unique<ShortLiteralToken>(pos, current(),
...
unique_ptr<const Token> Scanner::scanString() {
  ...
  if (str.length() <= 1) {
    unsigned char value =
        str.empty() ? '\0' : static_cast<unsigned char>(str[0]);
    return make_unique<CharLiteralToken>(pos, current(), value);
  }
